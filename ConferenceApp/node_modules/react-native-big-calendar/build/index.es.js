...
var _HourGuideColumn = function (_a) {
    ...
    var HourComp = HourComponent;
    return (React.createElement(View, __assign({ style: { height: cellHeight } }, calendarCellAccessibilityProps), HourComponent ? (React.createElement(HourComp, { hour: hour, ampm: ampm })) : (React.createElement(Text, { style: [objHasContent(hourStyle) ? hourStyle : textStyle, u['text-center']] }, formatHour(hour, ampm)))));
};
...
function _CalendarBodyForMonthView(_a) {
    ...
    var eventsByDate = React.useMemo(function () {
        var eventDict = {};
        if (!sortedMonthView) {
            var gridStart = dayjs(targetDate).startOf('month').startOf('week');
            var gridEnd = dayjs(targetDate).endOf('month').endOf('week');
            var d_1 = gridStart.clone();
            while (d_1.isBefore(gridEnd, 'day')) {
                var key = d_1.format(SIMPLE_DATE_FORMAT);
                eventDict[key] = events.filter(function (_a) {
                    var start = _a.start, end = _a.end;
                    // replicate your previous behavior: inclusive start, exclusive end
                    return d_1.isBetween(dayjs(start).startOf('day'), dayjs(end).endOf('day'), null, '[)');
                });
                d_1 = d_1.add(1, 'day');
            }
            return eventDict;
        }
        var dateToCompare = dayjs(targetDate).startOf('month').startOf('week').startOf('day');
        var startDateOfWeek = dateToCompare.startOf('week');
        var lastDateOfWeek = dateToCompare.endOf('week');
        var multipleDayEventsOrder = new Map();
        var lastDateOfMonth = dayjs(targetDate)
            .endOf('month')
            .endOf('week')
            .endOf('day')
            .add(1, 'day');
        var _loop_1 = function () {
            ...
            var todayStartsEvents = events
                .filter(function (event) {
                return dateToCompare.isSame(dayjs(event.start).startOf('day'), 'day') ||
            ...
            var todayStartsEventsSet = new Set(todayStartsEvents);
            var finalEvents = __spreadArray([], todayStartsEvents, true);
            // Import and sort events that don't start today, but are included today.
            var todayIncludedEvents = events
                .filter(function (event) {
                return dateToCompare.isBetween(dayjs(event.start).startOf('day'), dayjs(event.end).startOf('day'), 'day', '[]') && !todayStartsEventsSet.has(event);
            })
                .sort(function (a, b) { var _a, _b; return ((_a = multipleDayEventsOrder.get(a)) !== null && _a !== void 0 ? _a : 0) - ((_b = multipleDayEventsOrder.get(b)) !== null && _b !== void 0 ? _b : 0); });
            // Inserts an existing multi-day event into today's schedule,
            // preserving the order of the existing multi-day event.
            for (var _i = 0, todayIncludedEvents_1 = todayIncludedEvents; _i < todayIncludedEvents_1.length; _i++) {
                var event = todayIncludedEvents_1[_i];
                if (!multipleDayEventsOrder.has(event))
                    continue;
                var order = multipleDayEventsOrder.get(event);
                if (order === undefined)
                    continue;
    ...
    return (React.createElement(View, __assign({ style: [
        ...
        week
            ...
            var _a;
            return (React.createElement(TouchableOpacity, __assign({ onLongPress: function () { return date && onLongPressCell && onLongPressCell(date.toDate()); }, onPress: function () { return date && onPressCell && onPressCell(date.toDate()); }, style: [
                    i > 0 && u['border-t'],
                ...
                calendarWidth > 0 &&
                    ...
                    date &&
                    ((_a = eventsByDate[date.format('YYYY-MM-DD')]) !== null && _a !== void 0 ? _a : []).reduce(function (elements, event, index, events) { return __spreadArray(__spreadArray([], elements, true), [
...
function _CalendarContainer(_a) {
    ...
    var HeaderComp = HeaderComponent;
    return (React__default.createElement(React__default.Fragment, null,
        React__default.createElement(HeaderComp, __assign({}, headerProps)),
